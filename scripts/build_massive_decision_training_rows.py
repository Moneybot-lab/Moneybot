...
def _row_before(rows: list[dict[str, Any]], day: str) -> int | None:
    """Return the most recent completed market row before the event UTC day."""
    idx = None
...
def build_training_rows_from_raw_market(events: list[dict[str, Any]], market: dict[str, list[dict[str, Any]]], *, horizon_days: int = 5) -> tuple[list[dict[str, Any]], dict[str, int]]:
    ...
    for event in events:
        ...
        row = {
            ...
            "leakage_guard": "features_asof_prior_completed_market_close_before_decision_date_labels_after_feature_date",
        }
        rows.append(row)
...
def write_rows(path: Path, rows: list[dict[str, Any]], summary: dict[str, int], *, raw_root: Path, decision_log: Path, horizon_days: int) -> dict[str, Any]:
    ...
    manifest = {
        ...
        "leakage_safe": True,
        "join_policy": "last_completed_market_row_before_decision_utc_date; labels strictly after that row",
        **summary,
    }
