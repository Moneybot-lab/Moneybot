...
FORBIDDEN_EXACT = {
    "return_5d", "label_up_5d", "label_asof_date", "feature_return_5d",
    "probability_up", "model_version", "recommendation",
}
FORBIDDEN_FRAGMENTS = ("future_", "forward_", "outcome_", "realized_return", "label_")
MARKET_FEATURE_TOKENS = (
    "lagged", "return", "rsi", "macd", "sma", "ema", "vwap", "trend",
    "volume", "liquidity", "volatility", "atr", "regime", "spy", "sector",
    "relative", "price_to_", "moving_average", "beta",
)


def _load_jsonl(path: Path) -> pd.DataFrame:
    if not path.exists():
        raise FileNotFoundError(f"Required cleaned Massive input does not exist: {path}")
...
def _market_feature_columns(frame: pd.DataFrame) -> list[str]:
    ...
    for raw_name in frame.columns:
        name = str(raw_name)
        lowered = name.lower()
        if not lowered.startswith("feature_") or name in MODEL_ECHO_FEATURES or name in FORBIDDEN_EXACT:
            continue
        if any(fragment in lowered for fragment in FORBIDDEN_FRAGMENTS):
            continue
        if lowered.startswith("feature_endpoint_") or lowered.startswith("feature_source_") or lowered.startswith("feature_rec_"):
            continue
...
def _event_dates(frame: pd.DataFrame) -> pd.Series:
    if "event_date" in frame.columns:
        return pd.to_datetime(frame["event_date"], utc=True, errors="coerce").dt.normalize()
    return pd.to_datetime(pd.to_numeric(frame["ts"], errors="coerce"), unit="s", utc=True, errors="coerce").dt.normalize()


def _temporal_train_periods(frame: pd.DataFrame) -> tuple[list[pd.DataFrame], list[dict[str, Any]]]:
    ordered = frame.assign(_event_date=_event_dates(frame)).dropna(subset=["_event_date"]).sort_values("_event_date")
    dates = sorted(ordered["_event_date"].unique())
    if len(dates) < 21:
        raise ValueError("Massive baseline training requires at least 21 independent dates")
    ...
    date_sets = (set(dates[:fit_end]), set(dates[fit_end:calibration_end]), set(dates[calibration_end:]))
    periods = [ordered.loc[ordered["_event_date"].isin(values)].drop(columns="_event_date").copy() for values in date_sets]
    cleaned, diagnostics = purge_embargo_periods(periods, horizon_days=5, embargo_days=1)
    if any(period.empty for period in cleaned):
        raise ValueError("purge/embargo leaves an empty Massive fit, calibration, or threshold period")
...
def _duplicate_weights(frame: pd.DataFrame) -> np.ndarray:
    ...
    counts = keys.groupby(list(GROUP_COLUMNS), dropna=False)[GROUP_COLUMNS[0]].transform("size").astype(float)
    return (1.0 / counts).to_numpy(dtype=float)


def _fill_from_fit(fit: pd.DataFrame, others: list[pd.DataFrame], features: list[str]) -> tuple[pd.DataFrame, list[pd.DataFrame], dict[str, float]]:
    fills: dict[str, float] = {}
    fit_out = fit.copy()
    for feature in features:
        numeric = pd.to_numeric(fit_out[feature], errors="coerce").replace([np.inf, -np.inf], np.nan)
    ...
    for frame in others:
        out = frame.copy()
        for feature in features:
            values = pd.to_numeric(out.get(feature), errors="coerce").replace([np.inf, -np.inf], np.nan)
...
def _artifact(base: BaselineModelArtifact, features: list[str], *, version: str = VERSION, threshold: float | None = None) -> BaselineModelArtifact:
    return BaselineModelArtifact(
        ...
        bias=base.bias,
        decision_threshold=float(base.decision_threshold if threshold is None else threshold),
...
def _score(frame: pd.DataFrame, probs: np.ndarray, threshold: float, weights: np.ndarray | None = None) -> dict[str, Any]:
    ...
    row_weights = np.ones(len(frame), dtype=float) if weights is None else np.asarray(weights, dtype=float)
    ...
    symbols = frame["symbol"].fillna("unknown").astype(str) if "symbol" in frame.columns else pd.Series("unknown", index=frame.index)
    ...
    if len(selected_groups) and selected_weight_total > 0:
        grouped_weights = selected_groups.groupby(["symbol", "date"], dropna=False)["weight"].sum()
        top_concentration = float(grouped_weights.max() / grouped_weights.sum())
    else:
        top_concentration = 0.0
    avg_return = float(np.average(selected_returns, weights=selected_weights)) if selected_weight_total > 0 else None
    return {
        "rows": int(len(frame)),
        ...
        "big_gain_capture_rate": round(float((preds & big_gains).sum() / big_gains.sum()), 6) if big_gains.any() else None,
        "big_loss_rows": int(big_losses.sum()),
        "big_loss_predictions": int((preds & big_losses).sum()),
        "big_loss_false_positive_rate": round(float((preds & big_losses).sum() / big_losses.sum()), 6) if big_losses.any() else None,
        "selected_unique_symbols": int(symbols[preds].nunique()),
        "selected_unique_dates": int(dates[preds].nunique()),
        "selected_unique_symbol_dates": int(len(selected_groups[["symbol", "date"]].drop_duplicates())),
...
def _select_threshold(frame: pd.DataFrame, probs: np.ndarray) -> dict[str, Any]:
    ...
    selected = max(viable, key=lambda item: (item["avg_selected_return"], item["big_gain_capture_rate"])) if viable else None
    return {
        "selected_threshold": float(selected["threshold"]) if selected else 0.55,
        "threshold_selection_sufficient": bool(selected),
        "selection_status": "selected_supported_threshold" if selected else "insufficient_support_keep_default_threshold",
...
def train_massive_market_model(
    ...
    *,
    ...
    fit, [calibration, threshold, holdout], fills = _fill_from_fit(fit_raw, [calibration_raw, threshold_raw, test], features)
    ...
    for feature in features:
        values = pd.to_numeric(all_cleaned[feature], errors="coerce") if feature in all_cleaned.columns else pd.Series(np.nan, index=all_cleaned.index)
        feature_coverage["features"][feature] = {
            "non_null_count": int(values.notna().sum()),
            "availability_rate": round(float(values.notna().mean()), 6) if len(values) else 0.0,
    ...
    report = {
        ...
        "duplicate_weighting_applied": True,
        "training_inputs": {"train": str(train_path), "test": str(test_path), "all_cleaned": str(all_path)},
        "row_counts": {"train": int(len(train)), "test": int(len(test)), "all_cleaned": int(len(all_cleaned)), "fit": int(len(fit)), "calibration": int(len(calibration)), "threshold": int(len(threshold))},
        "temporal_validation": {"cleaned_test_untouched_for_final_holdout": True, "boundaries": boundaries},
        "calibration": calibration_report,
        "threshold_selection": threshold_report,
...
def main() -> None:
    ...
    parser.add_argument("--output", default="data/track_b/massive_baseline_model_v1.json")
    args = parser.parse_args()
    report = train_massive_baseline(Path(args.train), Path(args.test), Path(args.all_cleaned), Path(args.output))
