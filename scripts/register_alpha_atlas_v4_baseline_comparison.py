...
def main()->int:
 ...
 identity_dependencies=materiality.get('identity_dependency_report') or {"status":"UNKNOWN","reason":"REGISTRATION_DID_NOT_PRODUCE_IDENTITY_DEPENDENCIES"}
 scope_decision=report.get('comparison_scope_decision') or {"status":"UNAVAILABLE"}
 (a.output_dir/'identity_dependencies.json').write_text(json.dumps(identity_dependencies,indent=2,sort_keys=True)+'\n')
 (a.output_dir/'comparison_scope_decision.json').write_text(json.dumps(scope_decision,indent=2,sort_keys=True)+'\n')
 ...
 identity_md=f"# V4 identity dependencies\n\n- Development rows: `{identity_dependencies.get('development_rows_examined')}`.\n- Validation rows: `{identity_dependencies.get('validation_rows_examined')}`.\n- Unique tickers / observation keys: `{identity_dependencies.get('unique_tickers')}` / `{identity_dependencies.get('unique_listing_or_observation_keys')}`.\n- Row classifications: `{identity_dependencies.get('row_classification_counts')}`.\n- Validation rows affected: `{identity_dependencies.get('validation_affected_rows')}`.\n- Dependency occurrences: `{identity_dependencies.get('dependency_occurrence_counts')}`.\n- Window: `{identity_dependencies.get('overall_dependency_window')}`.\n- Same-security continuity: `{identity_dependencies.get('same_security_continuity')}`.\n\nFull canonical-ID details are in `identity_dependencies.json`.\n"
 scope_md=f"# V4 comparison-scope decision\n\n- Status: `{scope_decision.get('status')}`.\n- Recommended option: `{scope_decision.get('recommended_option')}`.\n- Approval boundary: `{scope_decision.get('approval_boundary')}`.\n\n## Option A\n\n`{scope_decision.get('option_a')}`\n\n## Option B\n\n`{scope_decision.get('option_b')}`\n"
 (a.output_dir/'identity_dependencies.md').write_text(identity_md); (a.output_dir/'comparison_scope_decision.md').write_text(scope_md)
