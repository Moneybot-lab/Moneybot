...
from moneybot.services.alpha_atlas_v4_narrow_diagnostic import DiagnosticSpecError, aggregate_candidate, compare_grouping_reproduction, grouping_audit, score_candidate_fold, validate_spec
...
def main() -> int:
    ...
    failure=None; implementation_commit=subprocess.check_output(["git","rev-parse","HEAD"],text=True).strip()
    validated_inputs={}; grouping_executed=False; reproduction_diagnostics=None
    try:
        ...
        validated_inputs={role:{"path":path.name,"sha256":_sha(path),**spec["frozen_inputs"][role].get("provenance",{})} for role,path in inputs.items()}
        approved=json.loads(a.approved_grouping_audit.read_text())
        audit_contract=spec.get("approved_grouping_audit") or {"rows":10273,"assignments":421193}
        expected_audit=("AUDIT_COMPLETE_NO_SCORING",a.reviewed_specification_hash,audit_contract["rows"],audit_contract["assignments"],True,False)
        ...
        assigned_ids={x["canonical_observation_id"] for x in assignments}; reproduced=grouping_audit([rows[x] for x in sorted(assigned_ids)],assignments); grouping_executed=True
        reproduction_diagnostics=compare_grouping_reproduction(approved,reproduced)
        ...
        result={"schema_version":"alpha-atlas-v4-narrow-frozen-sample-results.v1","execution_status":"COMPLETE","descriptive_findings_status":"CONDITIONAL_SAVED_SAMPLE_ONLY",
          ...
          "input_provenance":validated_inputs,"implementation_commit":implementation_commit,"grouping_reconciliation":{k:v for k,v in reproduced.items() if k!="row_to_group_mapping"},
          "grouping_reproduction_diagnostics":reproduction_diagnostics,
        ...
        failure=exc; result={"schema_version":"alpha-atlas-v4-narrow-frozen-sample-results.v1","execution_status":"FAILED","reason_code":getattr(exc,"code",type(exc).__name__),
          ...
          "mismatch_details":getattr(exc,"details",{}),"input_provenance":validated_inputs,"implementation_commit":implementation_commit,
          "grouping_executed":grouping_executed,"scoring_completed":False,"performance_scoring_executed":False,"holdout_content_loaded":False,"specification_sha256_expected":a.reviewed_specification_hash}
    ...
    if failure: lines += [f"- Failure: `{result['reason_code']}`.",f"- Reproduction details: `{result.get('mismatch_details')}`.",f"- Grouping / scoring completed: `{result.get('grouping_executed')}` / `false`."]
