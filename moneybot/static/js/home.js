const fallbackData = {
                  ...
                  function renderMomentum(items){
                    document.getElementById('momentum').innerHTML = `<table style="width:100%;border-collapse:collapse"><thead><tr><th style="text-align:left;padding:8px;border-bottom:1px solid #d1fae5">Ticker</th><th style="text-align:left;padding:8px;border-bottom:1px solid #d1fae5">Price</th><th style="text-align:left;padding:8px;border-bottom:1px solid #d1fae5">Score</th><th style="text-align:left;padding:8px;border-bottom:1px solid #d1fae5">Recommendation</th><th style="text-align:left;padding:8px;border-bottom:1px solid #d1fae5">Setup</th><th style="text-align:left;padding:8px;border-bottom:1px solid #d1fae5">Source</th><th style="text-align:left;padding:8px;border-bottom:1px solid #d1fae5">Transparency</th></tr></thead><tbody>${items.map(item=>`<tr><td style="padding:8px;border-bottom:1px solid #dcfce7">${tickerButton(item.symbol)}</td><td style="padding:8px;border-bottom:1px solid #dcfce7">${formatMoney(item.price)}</td><td style="padding:8px;border-bottom:1px solid #dcfce7">${item.score}</td><td style="padding:8px;border-bottom:1px solid #dcfce7"><span class="advice-badge buy">${item.recommendation}</span></td><td style="padding:8px;border-bottom:1px solid #dcfce7">${String(item.setup_type || '').replace('_', ' ')}</td><td style="padding:8px;border-bottom:1px solid #dcfce7">${item.decision_source || 'rule_based'}</td><td style="padding:8px;border-bottom:1px solid #dcfce7">${item.rationale}</td></tr>`).join('')}</tbody></table><p style="margin:10px 0 0 0;color:#166534;font-size:12px;font-weight:700">Click on advice badges to see why.</p>`;
                  }
                  ...
                  function renderBreakout(items){
                    ...
                    document.getElementById('breakout').innerHTML = `<table style="width:100%;border-collapse:collapse"><thead><tr><th style="text-align:left;padding:8px;border-bottom:1px solid #d1fae5">Ticker</th><th style="text-align:left;padding:8px;border-bottom:1px solid #d1fae5">Price</th><th style="text-align:left;padding:8px;border-bottom:1px solid #d1fae5">Score</th><th style="text-align:left;padding:8px;border-bottom:1px solid #d1fae5">Recommendation</th><th style="text-align:left;padding:8px;border-bottom:1px solid #d1fae5">Change</th><th style="text-align:left;padding:8px;border-bottom:1px solid #d1fae5">Rel. Volume</th><th style="text-align:left;padding:8px;border-bottom:1px solid #d1fae5">Scanner</th><th style="text-align:left;padding:8px;border-bottom:1px solid #d1fae5">Transparency</th></tr></thead><tbody>${rows.map(item=>`<tr><td style="padding:8px;border-bottom:1px solid #dcfce7">${tickerButton(item.symbol)}</td><td style="padding:8px;border-bottom:1px solid #dcfce7">${formatMoney(item.price)}</td><td style="padding:8px;border-bottom:1px solid #dcfce7">${item.score}</td><td style="padding:8px;border-bottom:1px solid #dcfce7"><span class="advice-badge buy">${item.recommendation}</span></td><td style="padding:8px;border-bottom:1px solid #dcfce7">${Number(item.change_percent || 0).toFixed(2)}%</td><td style="padding:8px;border-bottom:1px solid #dcfce7">${item.volume_ratio == null ? '—' : Number(item.volume_ratio).toFixed(1) + 'x'}</td><td style="padding:8px;border-bottom:1px solid #dcfce7">${item.decision_source || item.candidate_source || 'scanner'}</td><td style="padding:8px;border-bottom:1px solid #dcfce7">${item.rationale}</td></tr>`).join('')}</tbody></table><p style="margin:10px 0 0 0;color:#166534;font-size:12px;font-weight:700">Breakout Radar is powered by live small-cap, day-gainer, and recent breakout-alert candidates.</p>`;
                  }
                  ...
                  async function refreshTab(tab){
                    ...
                    try {
                      if(tab === 'stable'){
                        ...
                        const breakout = await fetchWithFallback('/api/breakout-radar', 'breakout');
