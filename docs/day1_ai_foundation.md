...
- treats model + threshold as the challenger artifact by evaluating thresholds 0.55, 0.575, 0.60, 0.625, 0.65, 0.675, and 0.70 on the training split, then selecting by the same profit-aware utility that the promotion gate evaluates with big-loss guardrails
