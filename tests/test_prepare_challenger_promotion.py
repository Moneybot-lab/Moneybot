...
def test_prepare_challenger_promotion_selects_gate_cleared_logistic_model(tmp_path):
    ...
    report = {
        ...
        "challengers": [
            ...
            },
            {
                ...
                "model_type": "logistic_regression",
                "model_path": str(model),
                "backtest_metrics": {"total_return_net": 0.02},
                "promotion_gates": {"promotion_ready": True},
                "routing_allowed": False,
            },
        ],
    ...
    result = prepare_challenger_promotion(
        backtest_report_path=report_path, output_dir=tmp_path / "out"
    )

    comparison = json.loads(
        (tmp_path / "out" / "model_comparison_track_b.json").read_text(encoding="utf-8")
    )
    candidate = json.loads(
        (tmp_path / "out" / "candidate_model_track_b.json").read_text(encoding="utf-8")
    )
...
def test_prepare_challenger_promotion_writes_losing_report_when_no_model_clears_gates(
    tmp_path,
):
    report_path = tmp_path / "backtest.json"
    report_path.write_text(
        json.dumps({"ranked_model_versions": [], "challengers": []}), encoding="utf-8"
    )
...
def test_prepare_challenger_promotion_rejects_offline_only_features(tmp_path):
    ...
    assert result["candidate_win"] is False
    assert result["unsupported_live_features"] == ["feature_close", "feature_volume"]
    assert candidate["version"] == "no-live-compatible-challenger"
