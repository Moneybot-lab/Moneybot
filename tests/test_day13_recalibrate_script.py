...
def test_build_recalibration_plan_applies_bounded_intercept_delta_when_rollout_enabled():
    report = {
        "rows": 120,
    ...
    assert plan["canary_mode"] is False
...
def test_build_recalibration_plan_skips_when_rows_below_threshold():
    ...
    plan = build_recalibration_plan(
        report, current_slope=1.0, current_intercept=0.1, min_rows=10
    )

    assert plan["apply_change"] is False
    assert plan["eligible_for_change"] is False
    assert plan["next"]["intercept"] == 0.1


def test_build_recalibration_plan_applies_bounded_slope_delta_when_rollout_enabled():
...
def test_build_recalibration_plan_includes_effective_brier_and_defaults_to_canary():
    report = {
        "rows": 120,
    ...
    plan = build_recalibration_plan(
        report, current_slope=1.0, current_intercept=0.0, min_rows=30
    )

    assert plan["apply_change"] is False
    assert plan["eligible_for_change"] is True
    assert plan["canary_mode"] is True
    assert plan["enforcement"] == "plan_only"
