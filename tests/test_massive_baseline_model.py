            ...
            output.append(
                {
                    ...
                    "label_asof_date": (date + pd.Timedelta(days=5)).strftime(
                        "%Y-%m-%d"
                    ),
...
def test_massive_baseline_trains_from_cleaned_inputs_and_excludes_unsafe_features(
    tmp_path,
):
    ...
    assert (
        artifact.lineage["sample_weight_policy"]
        == "1 / count(symbol, event_date, endpoint, decision_source)"
    )
    ...
    assert (
        ...
        == "1 when close(T+5 trading bars) / close(T) - 1 > 0; otherwise 0"
    )
    ...
    assert report["training_inputs"] == {
        "train": str(train_path),
        "test": str(test_path),
        "all_cleaned": str(all_path),
    }
    assert (
        report["temporal_validation"]["cleaned_test_untouched_for_final_holdout"]
        is True
    )
...
def test_feature_policy_rejects_future_echo_and_endpoint_fields():
    ...
    assert _market_feature_columns(frame) == [
        "feature_return_5d_lagged",
        "feature_rsi_14",
    ]


def test_massive_comparator_is_selected_but_legacy_default_fill_remains_diagnostic(
    tmp_path,
):
    ...
    save_artifact(
        BaselineModelArtifact(
            ...
            feature_columns=["feature_rsi_14"],
            means=[50.0],
            stds=[10.0],
            weights=[1.0],
            bias=0.0,
            decision_threshold=0.55,
            lineage={"target_column": "label_up_5d"},
        ),
        baseline,
    )

    resolved = _resolve_massive_comparator(
    ...
    valid = _comparison_scope_report(
        ...
        candidate_coverage,
        baseline_coverage,
        target_columns_match=True,
        duplicate_weighting_policy_match=True,
    )
    ...
    eligible = _comparison_promotion_evidence(
        valid,
        leakage_passed=True,
        threshold_support_passed=True,
        concentration_passed=True,
    )
    threshold_blocked = _comparison_promotion_evidence(
        valid,
        leakage_passed=True,
        threshold_support_passed=False,
        concentration_passed=True,
    )
